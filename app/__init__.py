...
def validate_runtime_config(app):
    ...
    if not secret_key or secret_key in {
        ...
        raise RuntimeError(
            "SECRET_KEY must be set to a strong private value in production."
        )
    ...
    if bool(mail_username) != bool(mail_password):
        warnings.warn(
            "MAIL_USERNAME and MAIL_PASSWORD should be set together.", stacklevel=2
        )

    admin_code = app.config.get("ADMIN_REGISTRATION_CODE")
    if not admin_code:
        raise RuntimeError(
            "ADMIN_REGISTRATION_CODE must be set when admin routes are enabled."
        )
    if len(admin_code) < 16:
        raise RuntimeError(
            "ADMIN_REGISTRATION_CODE must be at least 16 characters in production."
        )
    if admin_code and not app.config.get("ADMIN_ALERT_EMAIL"):
        warnings.warn(
            "ADMIN_ALERT_EMAIL should be set when admin registration is enabled.",
            stacklevel=2,
        )
...
def create_app(config_object=Config):
    ...
    def service_worker():
        ...
        resp = make_response(send_from_directory(app.static_folder + "/js", "sw.js"))
